...
function filterForLargeWords(arr, length) {
  const arrFiltered = arr.filter((word) => word.length > length);
  return arrFiltered;
}
...
function makeMirrorArray(arr) {
  const arrCopy = [...arr];
  const reverseArrCopy = arrCopy.reverse();
  reverseArrCopy.shift();
  const finalArray = arr.concat(reverseArrCopy);
  return finalArray;
}
